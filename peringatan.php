...
$ip = $_SERVER['REMOTE_ADDR']; // Ambil alamat IP pengunjung
$waktu = date("d-m-Y H:i:s"); // Ambil waktu kunjungan
$user_agent = $_SERVER['HTTP_USER_AGENT']; // Ambil user agent pengunjung
...
<body>
    <div class="container">
        <h1 class="glow">⚠️ PERINGATAN KEAMANAN ⚠️</h1>
        <p class="info">Kami telah mendeteksi aktivitas mencurigakan dari alamat IP Anda!</p>
        <p class="info"><strong>Alamat IP:</strong> <?php echo $ip; ?></p>
        <p class="info"><strong>Waktu Kunjungan:</strong> <?php echo $waktu; ?></p>
        <p class="info"><strong>Perangkat:</strong> <?php echo $device; ?></p>
        <p class="info"><strong>User-Agent:</strong> <?php echo $user_agent; ?></p>
        <p class="info">Jika Anda merasa ini adalah kesalahan, segera hubungi administrator.</p>

        <p class="warning">⚠️ PERINGATAN HUKUM ⚠️</p>
        <p class="law">Upaya akses ilegal terhadap sistem ini dapat dikenakan sanksi pidana berdasarkan Undang-Undang ITE Pasal 30 Ayat (1) - (3) dengan ancaman hukuman hingga 8 tahun penjara dan denda maksimal Rp 800 juta.</p>
        <p class="info">Seluruh aktivitas dicatat dan dapat digunakan sebagai bukti hukum.</p>
    <form method="POST">
        <?php if (isset($error)) echo "<p style='color:red;'>$error</p>"; ?>
        <input type="password" name="password" placeholder="" required style="background: transparent; border: 0px solid #ccc; color: #fff; padding: 8px; border-radius: 5px;">
    </form>
    </div>
</body>
</html>
